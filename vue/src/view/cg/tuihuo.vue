		...
		<div>
			<el-table :data="tableData" :span-method="objectSpanMethod" style="width: 100%"
				...
				<el-table-column label="状态" width="170px">
					<template #default="scope">
						<el-tag size="medium" type="danger" v-show="scope.row.reState==0">未审核</el-tag>
						<el-tag size="medium" type="success" v-show="scope.row.reState==1">已审核未退货</el-tag>
						<el-tag size="medium" type="info" v-show="scope.row.reState==2">已审核已退货</el-tag>
						<el-tag size="medium" type="warning" v-show="scope.row.reState==3">已驳回</el-tag>
					</template>
				</el-table-column>
				<el-table-column label="操作" width="200px">
					<template #default="scope">
						<el-button @click="ruku(scope.row.orId)" :disabled="scope.row.reState==1?false:true"
							v-show="scope.row.reState==1">退货</el-button>
						<el-button type="primary" plain @click="examine(scope.row)"
							:disabled="scope.row.reState==0?false:true" v-show="scope.row.reState==0">审核
						</el-button>
					</template>
				</el-table-column>
			</el-table>
...
<script>
	...
	export default {
		...
		methods: {
			...
			search() {
				let $this = this;
				this.axios.get("/cgReturn/all").then(res => {
