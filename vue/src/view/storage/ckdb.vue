<template>
	...
	<el-table :data="ckdbs" style="width: 100%;" border :header-cell-style="{'text-align':'center'}"
		...
		<el-table-column prop="dpBian" label="库存调拨编号" width="160"></el-table-column>
		<el-table-column prop="gId.gName" label="商品名称"></el-table-column>
		<el-table-column prop="dpNum" label="商品数量"></el-table-column>
		<el-table-column prop="whOut.whName" label="调出仓库" ></el-table-column>
		<el-table-column prop="whIn.whName" label="调入仓库"></el-table-column>
		...
		<el-table-column prop="dpState" label="状态" >
			<template #default="scope">
				...
				<p v-if="scope.row.dpState==1">通过</p>
				<p v-if="scope.row.dpState==2">已驳回</p>
			</template>
		</el-table-column>
		<el-table-column label="操作" width="120">
			<template #default="scope">
				...
				<el-tooltip effect="dark" content="不通过" placement="bottom">
					<i class="el-icon-close" @click="update(scope.row,2)" v-show="scope.row.dpState==0"
						style="font-size: 18px;cursor: pointer;margin-right: 25px;"></i>
				</el-tooltip>
			</template>
		</el-table-column>
